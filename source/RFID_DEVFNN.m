...
for i=1:ninput
    ...
    inputcovar(i,j)=temporary_IO(1,2);
     covarianceinput(i,j)=(covarianceinput_old(i,j)*(counter-1)+(((counter-1)/counter)*inputcovar(i,j)))/counter;
    end
end
            FCorrelation=zeros(ninput,noutput);
                       ...
                       for i=1:ninput
                       for j=1:noutput
                                                  pearson=covarianceinput(i,j)/sqrt(covarianceinput(j,j)*covarianceinput(i,i));
            FCorrelation(i,j)=(0.5*(covarianceinput(i,i)+covarianceinput(j,j))-sqrt((covarianceinput(j,j)+covarianceinput(i,i))^(2)-4*covarianceinput(i,i)*covarianceinput(j,j)*(1-pearson^(2))));
                       end
